...
class Requisition_lib {
    ...
    public function get_cart() {
        if (!$this->CI->session->userdata('REQ_cart')) {
            $this->set_cart(array());
        }
        return $this->CI->session->userdata('REQ_cart');
    }

    public function set_cart($cart_data) {
        $this->CI->session->set_userdata('REQ_cart', $cart_data);
    }

    public function empty_cart() {
        $this->CI->session->unset_userdata('REQ_cart');
    }

    public function get_supplier() {
        if (!$this->CI->session->userdata('REQ_supplier')) {
            $this->set_supplier(-1);
        }
        return $this->CI->session->userdata('REQ_supplier');
    }

    public function set_supplier($supplier_id) {
        $this->CI->session->set_userdata('REQ_supplier', $supplier_id);
    }

    public function remove_supplier() {
        $this->CI->session->unset_userdata('REQ_supplier');
    }

    public function get_mode() {
        if (!$this->CI->session->userdata('REQ_mode')) {
            $this->set_mode('receive');
        }
        return $this->CI->session->userdata('REQ_mode');
    }

    public function set_mode($mode) {
        $this->CI->session->set_userdata('REQ_mode', $mode);
    }

    public function clear_mode() {
        $this->CI->session->unset_userdata('REQ_mode');
    }
    
    public function get_currentid() {
        if (!$this->CI->session->userdata('REQ_currentid')) {
            return '';
        }
        return $this->CI->session->userdata('REQ_currentid');
    }

    public function set_currentid($po_id) {
        $this->CI->session->set_userdata('REQ_currentid', $po_id);
    }

    public function clear_currentid() {
        $this->CI->session->unset_userdata('REQ_currentid');
    }

    public function get_stock_source() {
        if (!$this->CI->session->userdata('REQ_stock_source')) {
            $this->set_stock_source(-1);
        }

        return $this->CI->session->userdata('REQ_stock_source');
    }

    public function get_comment() {
        // avoid returning a NULL that results in a 0 in the comment if nothing is set/available
        $comment = $this->CI->session->userdata('REQ_comment');

        return empty($comment) ? '' : $comment;
    }

    public function set_comment($comment) {
        $this->CI->session->set_userdata('REQ_comment', $comment);
    }

    public function clear_comment() {
        $this->CI->session->unset_userdata('REQ_comment');
    }

    public function get_reference() {
        return $this->CI->session->userdata('REQ_reference');
    }

    public function set_reference($reference) {
        $this->CI->session->set_userdata('REQ_reference', $reference);
    }

    public function clear_reference() {
        $this->CI->session->unset_userdata('REQ_reference');
    }

    public function is_print_after_sale() {
        return $this->CI->session->userdata('REQ_print_after_sale') == 'true' ||
                $this->CI->session->userdata('REQ_print_after_sale') == '1';
    }

    public function set_print_after_sale($print_after_sale) {
        return $this->CI->session->set_userdata('REQ_print_after_sale', $print_after_sale);
    }

    public function set_stock_source($stock_source) {
        $this->CI->session->set_userdata('REQ_stock_source', $stock_source);
    }

    public function clear_stock_source() {
        $this->CI->session->unset_userdata('REQ_stock_source');
    }

    public function get_stock_destination() {
        if (!$this->CI->session->userdata('REQ_stock_destination')) {
            $this->set_stock_destination($this->CI->Stock_location->get_default_location_id());
        }

        return $this->CI->session->userdata('REQ_stock_destination');
    }

    public function set_stock_destination($stock_destination) {
        $this->CI->session->set_userdata('REQ_stock_destination', $stock_destination);
    }

    public function clear_stock_destination() {
        $this->CI->session->unset_userdata('REQ_stock_destination');
    }

    public function add_item($stock_order_id,$received_qty='') {
        //make sure item exists in database.
        if (!$this->CI->Po->exists($stock_order_id,array('draft', 'open', 'partially', 'closed', 'void'),'stock_order')) {
            return FALSE;
        }
        $po_list = $this->CI->Po->get_po_items($stock_order_id,array('draft', 'open', 'partially', 'closed', 'void'),'stock_order');
        if (!$po_list) {
            return FALSE;
        } else {
            $this->empty_cart();
            ...
            foreach ($po_list as $key => $value) {
                ...
                //$this->set_stock_source($value['item_location']);
