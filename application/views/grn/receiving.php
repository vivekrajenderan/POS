...
<div id="page_title" class="text-center">
    <?php echo $this->lang->line('module_grn_order'); ?>
    <?php echo anchor($controller_name . "/lists", $this->lang->line('grn_list'), array('class' => 'btn btn-info btn-sm  pull-right', 'id' => 'show_sales_button', 'style' => 'margin-left: 3px;')); ?>
        ...
        <ul>
            <li class="pull-left first_li">
                <label for="item" class='control-label'>
                    <?php echo $this->lang->line('grn_find_or_scan_po'); ?>
                </label>
            </li>
            <li class="pull-left">
            ...
            <?php if (isset($po_id) && $po_id != '') { ?>
                <li class="pull-left">
                    <label for="item" class='control-label'>
                        <?php echo $this->lang->line('grn_po_id') . ' : ' . $po_id; ?> 
                    </label>
                </li>
            <?php } ?>
    ...
    <table class="sales_table_100" id="register">
        <thead>
            <tr>
                ...
                <th style="width:5%;" class="hidden"></th>
        ...
        <tbody id="cart_contents">
            ...
            if (count($cart) == 0) {
                ...
                <tr>
                    <td colspan='8'>
                        <div class='alert alert-dismissible alert-info'><?php echo $this->lang->line('grn_no_bill_in_cart'); ?></div>
                    </td>
                </tr>
                <?php
            } else {
                foreach (array_reverse($cart, TRUE) as $line => $item) {
                    ...
                    <tr>
                        ...
                        <td data-parent="<?php echo 'cart_' . $line; ?>"><?php echo form_input(array('name' => 'price', 'class' => 'form-control input-sm receiving_quantity', 'value' => to_currency_no_money($item['price']))); ?></td>


                        <td><?php
                            echo form_input(array('name' => 'requested_quantity', 'class' => 'form-control input-sm quantity hidden', 'value' => to_quantity_decimals($item['balance_quantity'])));
                            echo to_quantity_decimals($item['received_quantity']);
                            ?></td>
                        <td class='hidden'><?php echo form_input(array('name' => 'quantity', 'class' => 'form-control input-sm quantity', 'value' => to_quantity_decimals($item['quantity']))); ?></td>
                        ...
                        ?>
                        <td><?php echo to_currency($item['price'] * $item['receiving_quantity'] - $item['price'] * $item['receiving_quantity'] * $item['discount'] / 100); ?></td>
...
<div id="overall_sale" class="panel panel-default">
    <div class="panel-body">
        ...
        if (isset($supplier)) {
            ...
            <?php echo anchor($controller_name . "/remove_supplier", '<span class="glyphicon glyphicon-remove">&nbsp</span>' . $this->lang->line('common_remove') . ' ' . $this->lang->line('suppliers_supplier'), array('class' => 'btn btn-danger btn-sm hidden', 'id' => 'remove_supplier_button', 'title' => $this->lang->line('common_remove') . ' ' . $this->lang->line('suppliers_supplier')));
            ?>
            <?php
        } else {
            ?>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        $("#item").autocomplete(
                {
                    source: '<?php echo site_url($controller_name . "/po_search"); ?>',
                    minChars: 0,
                    delay: 10,
                    autoFocus: false,
