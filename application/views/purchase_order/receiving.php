...
<div id="page_title" class="text-center">
    <?php echo $this->lang->line('module_purchase_order'); ?>
    <?php echo anchor("purchase_order/lists", $this->lang->line('reports_purchase_order_history'), array('class' => 'btn btn-info btn-sm  pull-right', 'id' => 'show_sales_button', 'style' => 'margin-left: 3px;')); ?>
...
<div id="register_wrapper">
    ...
    if ($show_stock_locations) {
        ...
        <div class="panel-body form-group">
            <ul>
                <li class="pull-left first_li hidden">
    ...
    <div class="panel-body form-group">
        <ul>
            ...
            <!--            <li class="pull-right">
                            <a class='btn btn-info btn-sm pull-right' href='<?php echo site_url($controller_name . "/lists"); ?>'>
                                <span class="glyphicon glyphicon-list">&nbsp</span><?php echo $this->lang->line('po_history'); ?></a>
                        </li>-->
            <li class="pull-right">
    ...
    <table class="sales_table_100" id="register">
        <thead>
            <tr>
                ...
                <th style="width:10%;" class="hidden" ><?php echo $this->lang->line('receivings_cost'); ?></th>
                <th style="width:10%;"><?php echo $this->lang->line('receivings_quantity'); ?></th>
                <th style="width:10%;" class="hidden" ><?php echo $this->lang->line('receivings_discount'); ?></th>
                <th style="width:10%;" class="hidden"><?php echo $this->lang->line('receivings_total'); ?></th>
                ...
                foreach (array_reverse($cart, TRUE) as $line => $item) {
                    ...
                    <tr>
                        ...
                        if ($items_module_allowed && $mode != 'requisition') {
                            ?>
                            <td class="hidden"><?php echo form_input(array('name' => 'price', 'class' => 'form-control input-sm', 'value' => to_currency_no_money($item['price']))); ?></td>
                            <?php
                        } else {
                            ?>
                            <td class="hidden">
                        ...
                        if ($item['receiving_quantity'] > 1) {
                            ?>
                            <td class="hidden"><?php echo 'x' . to_quantity_decimals($item['receiving_quantity']); ?></td>	
                            <?php
                        } else {
                            ?>
                            <td></td>
                        ...
                        if ($items_module_allowed && $mode != 'requisition') {
                            ?>
                            <td class="hidden"><?php echo form_input(array('name' => 'discount', 'class' => 'form-control input-sm', 'value' => $item['discount'])); ?></td>
                            <?php
                        } else {
                            ?>
                            <td class="hidden"><?php echo $item['discount']; ?></td>
                            <?php echo form_hidden('discount', $item['discount']); ?>
                            <?php
                        }
                        ?>
                        <td class="hidden"><?php echo to_currency($item['price'] * $item['quantity'] - $item['price'] * $item['quantity'] * $item['discount'] / 100); ?></td>
...
<div id="overall_sale" class="panel panel-default">
    <div class="panel-body">
        ...
        if (isset($supplier)) {
            ...
            <?php echo anchor($controller_name . "/remove_supplier", '<span class="glyphicon glyphicon-remove">&nbsp</span>' . $this->lang->line('common_remove') . ' ' . $this->lang->line('suppliers_supplier'), array('class' => 'btn btn-danger btn-sm', 'id' => 'remove_supplier_button', 'title' => $this->lang->line('common_remove') . ' ' . $this->lang->line('suppliers_supplier')));
            ?>
            <?php
        } else {
            ?>
        ...
        <table class="sales_table_100" id="sale_totals">
            <tr>
                ...
                if ($mode != 'requisition') {
                    ?>
                    <th style="width: 55%;" class="hidden"><?php echo $this->lang->line('sales_total'); ?></th>
                    <th style="width: 45%; text-align: right;" class="hidden"><?php echo to_currency($total); ?></th>
                    <?php
                } else {
                    ?>
        ...
        if (count($cart) > 0) {
            ...
            <div id="finish_sale">
                ...
                if ($mode == 'po') {
                    ?>
                    <?php echo form_open($controller_name . "/po_complete", array('id' => 'finish_receiving_form', 'class' => 'form-horizontal')); ?>
