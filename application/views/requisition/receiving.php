...
<div id="page_title" class="text-center">
    <?php echo $this->lang->line('module_requisition');?>
    <?php echo anchor($controller_name."/lists",  $this->lang->line('requisition_list'), array('class' => 'btn btn-info btn-sm  pull-right', 'id' => 'show_sales_button','style'=>'margin-left: 3px;')); ?>
        ...
        <div class="panel-body form-group">
            <ul>
                ...
                if ($show_stock_locations) {
                    ...
                    <li class="pull-left">
                        <?php echo form_dropdown('stock_source', $stock_locations, $stock_source, array('id' => "stock_source", 'onchange' => "$('#mode_form').submit();", 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-default btn-sm', 'data-width' => 'fit')); ?>
                    </li>
        ...
        <ul>
            <li class="pull-left first_li">
                <label for="item" class='control-label'>
                    <?php echo $this->lang->line('requisition_find_or_scan_so'); ?>
                </label>
            </li>
            <li class="pull-left">
            ...
            <?php if ((isset($stock_order_id) && $stock_order_id != '') && count($cart) > 0) { ?>
                <li class="pull-left">
                    <label for="item" class='control-label'>
                        <?php echo $this->lang->line('requisition_so_ref') . ' : ' . $stock_order_id; ?> 
                    </label>
                </li>
            <?php } ?>            
        </ul>
    ...
    <table class="sales_table_100" id="register">
        ...
        <tbody id="cart_contents">
            ...
            if (count($cart) == 0) {
                ...
                <tr>
                    <td colspan='8'>
                        <div class='alert alert-dismissible alert-info'><?php echo $this->lang->line('requisition_no_so_in_cart'); ?></div>
                    </td>
                </tr>
                <?php
            } else {
        ...
        <div id="finish_sale">
            ...
            <div class="form-group form-group-sm">
                ...
                if (count($cart) > 0) {
                    ...
                    <div class='btn btn-sm btn-success pull-right' id='finish_receiving_button'><span class="glyphicon glyphicon-ok">&nbsp</span><?php echo $this->lang->line('requisition_send') ?></div>
                    <?php
                }
                ?>
            </div>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        ...
        $('#item').blur(function ()
        {
            $(this).attr('value', "<?php echo $this->lang->line('requisition_start_typing_so_ref'); ?>");
        });
    ...
    <?php if (count($stock_locations) > 0 && $stock_source == -1) { ?>
        <script>
                    $(document).ready(function() {
                $("#stock_source").trigger("change");
            });
        </script>
    <?php } ?>
