...
class Supplier extends Person {
    ...
    /*
     * Return all supplier for dropdown
     */

    public function get_dropdown($limit_from = 0, $rows = 0) {
        $supplier_array=$this->get_all()->result_array();
        $supplier = array();
        foreach ($supplier_array as $location_data) {
            $supplier[$location_data['person_id']] = $location_data['first_name']." ".$location_data['last_name'];
        }
        return $supplier;
    }
    /*
     * Return all To pay for dropdown
     */

    public function get_to_pay_dropdown($limit_from = 0, $rows = 0) {
        
        $this->db->from('suppliers');
        $this->db->join('people', 'suppliers.person_id = people.person_id');
        $this->db->join('receivings', 'suppliers.person_id = receivings.supplier_id and  receivings.receiving_mode = "receive" ');
        $this->db->where_in('receivings.payment_status', array('unpaid','partially'));
        $this->db->where('deleted', 0);
        $this->db->order_by('company_name', 'asc');
        $supplier_list = $this->db->get()->result_array();
        $supplier = array();
        foreach ($supplier_list as $location_data) {
            $supplier[$location_data['person_id']] = $location_data['company_name'];
        }
        return $supplier;
    }
    /*
     * Return all To pay for supplier id
     */

    public function get_to_pay_supplier_id($limit_from = 0, $rows = 0) {
        ...
        $this->db->limit(1, 0);
        $supplier_list = $this->db->get()->result_array();
        return isset($supplier_list[0])?$supplier_list[0]['person_id']:-1;
    }
