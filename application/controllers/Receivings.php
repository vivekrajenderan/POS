...
class Receivings extends Secure_Controller {
    ...
    public function complete() {
        ...
        if (isset($data['mode'])){
            $dy_prefix = strtoupper($data['mode']) . ' ';
        }
        if (isset($data['mode']) && $data['mode'] = 'po') {
            $data['receiving_id'] = $dy_prefix . $this->Receiving->savepo($data['cart'], $supplier_id, $employee_id, $data['comment'], $data['reference'], $data['payment_type'], $data['stock_location'], $dy_prefix);
        } else {
            $data['receiving_id'] = $dy_prefix . $this->Receiving->save($data['cart'], $supplier_id, $employee_id, $data['comment'], $data['reference'], $data['payment_type'], $data['stock_location'], $dy_prefix);
        }
    
        $data = $this->xss_clean($data);

        if ($data['receiving_id'] == $dy_prefix . '-1') {
