...
class Reports extends Secure_Controller
{
	public function __construct()
	{
		...
		if(sizeof($exploder) > 1)
		{
			...
			if(!$this->Employee->has_grant('reports_' . $submodule_id, $this->Employee->get_logged_in_employee_info()->role_id))
			{
	...
	public function specific_customer($start_date, $end_date, $customer_id, $sale_type)
	{
		...
		foreach($report_data['summary'] as $key => $row)
		{
			$summary_data[] = $this->xss_clean(array(
				'id' => anchor('sales/receipt/'.$row['sale_id'], $this->Appconfig->get('invoice_number_prefix').$row['sale_id'], array('target'=>'_blank')),
				'sale_date' => $row['sale_date'],
