...
class Taxes extends Secure_Controller {
    ...
    public function summary_taxes($start_date = NULL, $end_date = NULL) {
        $inputs = array('start_date' => (!empty($start_date)) ? $start_date : date('Y-m-01'),
            'end_date' => (!empty($end_date)) ? $end_date : date('Y-m-t')
        );
        $report_data = $this->Tax->getData($inputs);
        $summary = $this->xss_clean($this->Tax->getSummaryData($inputs));

        $tabular_data = array();
        foreach ($report_data as $row) {
            $tabular_data[] = $this->xss_clean(array(
                'tax_percent' => "<a href='" . site_url('taxes') . "/summary_tax_view/" . $row['item_id'] . "'>" . $row['percent'] . "</a>",
        ...
        $data = array(
            ...
            'headers' => $this->xss_clean($this->Tax->getDataColumns()),
            'data' => $tabular_data,
            'summary_data' => $summary
        );

        $this->load->view('taxes/taxes_summary', $data);
    }

    public function summary_taxes_detail($start_date = NULL, $end_date = NULL) {
        $inputs = array('start_date' => (!empty($start_date)) ? $start_date : date('Y-m-01'),
            'end_date' => (!empty($end_date)) ? $end_date : date('Y-m-t')
        );
        ...
        foreach ($report_data as $row) {
            ...
            $getSingleData = $this->Tax->getSingleData($row['item_id'], $inputs);
            foreach ($getSingleData as $drow) {
                $details_data[$row['item_id']][] = $this->xss_clean(array($drow['percent'], $drow['subtotal'], $drow['tax'], $drow['total']));
            }
        }
        ...
        $this->load->view('taxes/taxes_summary_details', $data);
    }

    public function summary_tax_view($item_id, $start_date = NULL, $end_date = NULL) {
        if (!empty($item_id)) {
            $inputs = array('start_date' => (!empty($start_date)) ? $start_date : date('Y-m-01'),
                'end_date' => (!empty($end_date)) ? $end_date : date('Y-m-t'),
                'item_id'=>$item_id
            );
            $report_data =  $this->Tax->getSingleData($item_id, $inputs);
            $summary = $this->xss_clean($this->Tax->getSummaryData($inputs));
            ...
            $data = array(
                'title' => $this->lang->line('reports_taxes_summary_report'),
                'headers' => $this->xss_clean($this->Tax->getViewDataColumns()),
                'data' => $tabular_data,
                'summary_data' => $summary,
                'item_id'=>$item_id
            );

            $this->load->view('taxes/taxes_summary_view', $data);
        }
    }
