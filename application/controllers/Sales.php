	...
	public function complete()
	{
		...
		if($this->sale_lib->is_invoice_mode() || $data['invoice_number_enabled'] == TRUE)
		{
			...
			else
			{
				...
				$data['sale_id'] = $this->Appconfig->get('invoice_number_prefix') . $data['sale_id_num'];
...
//				$this->suspend_quote($quote_number);
	...
	public function send_invoice($sale_id)
	{
		...
		if(!empty($sale_data['customer_email']))
		{
			...
			$text = str_replace('$CO', $this->Appconfig->get('invoice_number_prefix') . $sale_data['sale_id'], $text);
...
//		$data['total'] = $this->sale_lib->get_total();
		...
		$data['sale_id'] = $this->Appconfig->get('invoice_number_prefix') . $sale_id;
