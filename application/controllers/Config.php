...
class Config extends Secure_Controller {
    ...
    private function _licenses() {
        ...
        $license[$i]['title'] = 'Sokxay Minimart ' . $this->config->item('application_version');
    ...
    public function index() {
        $data['stock_locations'] = $this->Stock_location->get_all('store')->result_array();
        $data['warehouse'] = $this->Stock_location->get_all('warehouse')->result_array();
    ...
    public function warehouse() {
        $warehouse = $this->Stock_location->get_all('warehouse')->result_array();

        $warehouse = $this->xss_clean($warehouse);

        $this->load->view('partial/warehouse', array('warehouse' => $warehouse));
    }
    ...
    public function save_locations($storefor = 'store') {
        $this->db->trans_start();
        $deleted_locations = $this->Stock_location->get_allowed_locations('items', $storefor);
        $location_data = array();
        $insert_array = array(
            'name' => 'location_name',
            'phone' => 'phone_number',
        ...
        foreach ($this->input->post() as $key => $value) {
            ...
            $objectdata = explode('_', $key);
            if (!isset($location_data[$location_id]))
                $location_data[$location_id] = array();

            if (isset($location_data[$location_id]) && isset($objectdata[1])) {
                $location_data[$location_id][$insert_array[$objectdata[1]]] = $value;
            }
            unset($deleted_locations[$location_id]);
        }
        if (count($location_data) > 0) {
            foreach ($location_data as $key => $value) {
                if (isset($location_data[$key]) && count($location_data[$key]) > 0) {
                    if ($this->Stock_location->save($location_data[$key], $key, $storefor)) {
    ...
    public function save_locations1() {
        $this->db->trans_start();
    ...
    public function save_invoice() {
        $batch_save_data = array(
            ...
            'invoice_number_prefix' => $this->input->post('invoice_number_prefix'),
