...
class Items extends Secure_Controller {
    ...
    public function suggest_search() {
        $suggestions = $this->xss_clean($this->Item->get_search_suggestions($this->input->post_get('term'), array('search_custom' => $this->input->post('search_custom'), 'is_deleted' => $this->input->post('is_deleted') != NULL), FALSE));

        echo json_encode($suggestions);
    }

    public function suggest() {
        $suggestions = $this->xss_clean($this->Item->get_search_suggestions($this->input->post_get('term'), array('search_custom' => FALSE, 'is_deleted' => FALSE), TRUE));

        echo json_encode($suggestions);
    }

    public function suggest_kits() {
        $suggestions = $this->xss_clean($this->Item->get_kit_search_suggestions($this->input->post_get('term'), array('search_custom' => FALSE, 'is_deleted' => FALSE), TRUE));
    ...
    public function view($item_id = -1) {
        ...
        $data['item_kits_enabled'] = $this->Employee->has_grant('item_kits', $this->Employee->get_logged_in_employee_info()->role_id);
        ...
        foreach ($stock_locations as $location) {
            ...
            $price = $this->xss_clean($this->Item_price->get_item_price($item_id, $location['location_id'])->price);
            $price = ($item_id == -1) ? 0 : $price;

            $location_array[$location['location_id']] = array('location_name' => $location['location_name'], 'quantity' => $quantity, 'price' => $price);
            $data['stock_locations'] = $location_array;
        }
        $warehouse = $this->Stock_location->get_undeleted_all('items', 'warehouse')->result_array();
        foreach ($warehouse as $house) {
            $house = $this->xss_clean($house);

            $quantity = $this->xss_clean($this->Item_quantity->get_item_quantity($item_id, $house['location_id'])->quantity);
            $quantity = ($item_id == -1) ? 0 : $quantity;

            $price = $this->xss_clean($this->Item_price->get_item_price($item_id, $house['location_id'])->price);
            $price = ($item_id == -1) ? 0 : $price;

            $warehouse_array[$house['location_id']] = array('location_name' => $house['location_name'], 'quantity' => $quantity, 'price' => $price);
            $data['warehouse'] = $warehouse_array;
        }
    ...
    public function inventory($item_id = -1) {
        ...
        $data['adjustments_list'] = $this->Inventory->get_inventoryadjustment_dd();
        $data['adjustments'] = '2';
    ...
    public function save($item_id = -1) {
        ...
        $item_data = array(
            ...
            'item_type' => $this->input->post('item_type') == '' ? '0' : $this->input->post('item_type'),
            'stock_type' => $this->input->post('stock_type') == '' ? '0' : $this->input->post('stock_type'),
        ...
        if ($this->Item->save($item_data, $item_id)) {
            ...
            if ($item_id == -1) {
                ...
                InjAuditLog(array(
                    'action' => $this->lang->line('items_new'),
                    'ref_text' => $this->input->post('item_number') == '' ? '' : $this->input->post('item_number'),
                    'ref_id' => $item_id,
                    'url' => uri_string()
                ));
                $new_item = TRUE;
                ...
                InjAuditLog(array(
                    'action' => $this->lang->line('items_update'),
                    'ref_text' => $this->input->post('item_number') == '' ? '' : $this->input->post('item_number'),
                    'ref_id' => $item_id,
                    'url' => uri_string()
                ));
            ...
            $stock_locations = $this->Stock_location->get_undeleted_all()->result_array();
            $this->store_warehouse_update($item_id, $stock_locations, $new_item);
            $warehouse = $this->Stock_location->get_undeleted_all('items', 'warehouse')->result_array();
            $this->store_warehouse_update($item_id, $warehouse, $new_item, 'warehouse');
    ...
    /* Store and warehouse price and quantity update */

    public function store_warehouse_update($item_id, $stock_locations, $new_item, $locationtype = 'store') {
        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        foreach ($stock_locations as $location) {
            $updated_quantity = parse_decimals($this->input->post($locationtype . 'quantity_' . $location['location_id']));
            $updated_quantity = $updated_quantity > 0 ? $updated_quantity : 0;
            ...
            if ($item_quantity->quantity != $updated_quantity || $new_item) {
                InjAuditLog(array(
                    'action' => $this->lang->line('items_manually_editing_of_quantity'),
                    'ref_text' => $updated_quantity - $item_quantity->quantity,
                    'ref_id' => $item_id,
                    'location' => $location['location_id'],
                    'url' => uri_string()
                ));
                $this->Item_quantity->save($location_detail, $item_id, $location['location_id']);
            ...
            $updated_price = parse_decimals($this->input->post($locationtype . 'price_' . $location['location_id']));
            $updated_price = $updated_price > 0 ? $updated_price : 0;
            $location_detail = array('item_id' => $item_id,
                'location_id' => $location['location_id'],
                'price' => $updated_price);
            $item_price = $this->Item_price->get_item_price($item_id, $location['location_id']);
            if ($item_price->price != $updated_price || $new_item) {
                $this->Item_price->save($location_detail, $item_id, $location['location_id']);
            }


            $account_trans_data = array('amount' => $updated_price * $updated_quantity,
                'account_id' => '9',
                'reference' => $this->lang->line('items_manually_editing_of_quantity'),
                'reference_type' => 'items_manually',
                'reference_table' => 'items',
                'reference_id' => $item_id,
                'location_id' => $location['location_id'],
                'trans_type' => 'debit',
                'url' => 'inventories/details' . $item_id . '/' . $location['location_id']
            );
            $this->Account->insert_trans($account_trans_data);
            $account_trans_data['trans_type'] = 'credit';
            $account_trans_data['account_id'] = '22';
            $this->Account->insert_trans($account_trans_data);
        }
    }
    ...
    public function save_inventory($item_id = -1) {
        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        $cur_item_info = $this->Item->get_info($item_id);
        $this->form_validation->set_rules('newquantity', 'lang:items_add_minus', 'trim|required');
        $this->form_validation->set_rules('adjustments', 'lang:items_reason', 'trim|required');
        if ($this->form_validation->run() != FALSE && $item_id != -1) {
            ...
            $item_price = $this->Item_price->get_item_price($item_id, $location_id);
            ...
            if ($this->input->post('newquantity') < 0) {
                $account_trans_data = array('amount' => $this->input->post('newquantity') * $item_price->price * -1,
                    'account_id' => '9',
                    'reference' => $this->input->post('trans_comment'),
                    'reference_type' => 'InventoryAdjustments',
                    'reference_table' => 'inventory',
                    'reference_id' => $item_id,
                    'location_id' => $location_id,
                    'trans_type' => 'credit',
                    'url' => 'inventories/details/' . $item_id . '/' . $location_id
                );
                $this->Account->insert_trans($account_trans_data);
                $account_trans_data['trans_type'] = 'debit';
                $account_trans_data['account_id'] = '30';
                $this->Account->insert_trans($account_trans_data);
            }
            ...
            echo json_encode(array('success' => FALSE, 'error' => $this->form_validation->error_array(), 'id' => -1));
        }
    }
